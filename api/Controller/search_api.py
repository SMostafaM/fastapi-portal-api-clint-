...
form_archive_collection = db.get_collection("form_archive")
...
async def index(Xtoken: str = Header(""),Ujtoken:str=Header(""),type_req:str="all",data:str="all",page:str=1):
    ...
    if Xtoken=="0041e6ad98c3ae252dcca3f50e82fdf2d57a65fc":
        ...
        log_collection.insert_one({"user":user_info_token_username,"time":datetime.datetime.now(),"action":"search","subject":type_req,"object":data})
        return posts_list
    else:
...
async def post_count(type_req:str="notif",data:str="all",Xtoken: str = Header(""),Ujtoken:str=Header("")):
    user_info_token = jwt.decode(Ujtoken, SECRET_KEY_USERNAME, algorithms=[ALGORITHM_USERNAME])
    user_info_token_username=user_info_token["username"]
    if Xtoken=="0041e6ad98c3ae252dcca3f50e82fdf2d57a65fc":
        # if type_req == "notif":
